...
- Expected behavior: the system answers that the requested metric is not supported in the current ChatBI version, and either asks the user to refine the question or points to the four approved tools in `specs/08-chatbi-tool-schemas.md`. The backend does not synthesize an answer from PO-item count rows when the requested metric is materially different.
- Unacceptable behavior: returning a fabricated price-trend, value-weighted, quality-risk, or delivery-risk result; silently substituting PO item count for an unrelated measure; or asking the LLM to invent a calculation.
- Evidence required: captured user question, LLM intent JSON, backend rejection or clarification response, and answer text.
...
- Expected behavior: the result payload includes `source_trace` with at least dataset identifier, dataset version or refresh marker, tool name, filters applied, and time range applied. The frontend tooltip and answer text rely on `source_trace` rather than asking the LLM to describe lineage.
- Unacceptable behavior: omitting `source_trace`, populating it with free-form text generated by the LLM, or relying on the LLM to claim where the number came from.
- Evidence required: result JSON payload, frontend rendering of source metadata, and backend log of the tool call.
